...
from classifier.bn_class import *
...
if UCR:
  dataset = "BeetleFly"
  datadir = '/home/happyling/workspace/timeseries/data/UCR_TS_Archive_2015/' + dataset + '/' + dataset
...
#                  tf.nn.l2_loss(W_fc2) + tf.nn.l2_loss(b_fc2))
    ...
    #haha = tf.py_func(debug_two, [h_fc2, y_], [tf.bool])
    #with tf.control_dependencies(haha):
        #tmp_shape = y_.get_shape()
        #y_ = tf.reshape(y_, [tmp_shape[0], -1])
    loss = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=h_fc2, labels=y_)
    cost = tf.reduce_sum(loss) / batch_size
    #debug_cost = tf.py_func(debug_single, [cost], [tf.bool])
